...
int main(int argc, char* argv[]){
	...
	if (use_parsing_interface) cerr << "Using ParserInterface" << endl;
	...
	if (file.size()>3 && file[file.size()-3]=='.' && (file[file.size()-2]=='g' || file[file.size()-2]=='x' )&& file[file.size()-1]=='z') {
		...
		if (!use_parsing_interface) {
			...
			readStatus = parser_if.read_file_init_interface(in);
		}
	...
	if (!fo) {
		...
		if (!use_parsing_interface) {
			...
			assert(problemType == 1);
			readStatus = parser_if.read_file_init_interface(instanceFile);
		}
		instanceFile.close();
	}
	...
	int outputFormat;
	// This will nto be pretty... 
	if (!use_parsing_interface) {
		outputFormat = parseOutputFormat(flags, inputReader.inputFormat, inputReader.inputFormat, verb, iverb);
	...
	else { // use_parsing_interface
		assert(problemType == 1);
		outputFormat = maxPreprocessor::INPUT_FORMAT_WPMS22;
		maxPreprocessor::Timer preprocessTimer;
		preprocessTimer.start();
		...
		preprocessTimer.stop();
		if (verb > 0) parser_if.printPreprocessorStats(cerr);
		
		if (verb>0) cerr<<"Preprocess time: "<<preprocessTimer.getTime().count()<<endl;
		if (verb > 0) parser_if.printTimeLog(cerr);
		if (type == "preprocess") {	
			...
			parser_if.printInstance(cout, outputFormat);

			if (flags.count("mapfile")) {
				cerr<<"WARNING: separate output for reconstruction file currently not supported with parsing interface" << endl;
			}
			else {
				if (iverb>0) cout<<"c No -mapfile= given, will not ouput reconstruction map"<<endl;
				if (verb>0)  cerr<<"No -mapfile= given, will not ouput reconstruction map"<<endl;
		...
		if (type == "solve") {
			...
			if (verb>0)  cerr << "Saving preprocessed instance into file " << prepFile << endl;
			ofstream out(prepFile);
			parser_if.printInstance(out, outputFormat);
			out.close();
			...
			if (opr.status == 2) {
				...
				parser_if.printSolution(opr.trueLits, cout, opr.ansW, solutionFormat);
			}
			if (verb > 0) cerr<<"Preprocess time: "<<preprocessTimer.getTime().count()<<", Solve time: "<<solveTimer.getTime().count()<<endl;
			if (verb > 0) parser_if.printTimeLog(cerr);
		}
